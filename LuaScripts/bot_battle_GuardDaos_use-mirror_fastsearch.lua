...
local max_frame = 435000 --戦闘終了時（敵HPが0になった瞬間）の基準フレーム
local max_search = 50 --戦闘開始を遅らせて試行を始める最大フレーム数
...
function pattern_01()
	...
	--行動原理はディオス戦でのHPの減りを考慮してある程度調整
	
	--主人公、アグロス、ジュリナが倒れているまたはHPが減少している場合はミラクロースを使用する確率を上げる
	
	if p4hp == 0 or p4hp < 180 then
		r = math.random(3)
		if r >= 3 then
			miracle_02()
			return
	...
	--PT全員のHPが半分より減っているかつ、死者がいない場合はエストナを使用する確率を上げる
	if p1hp_max - p1hp > p1hp_max / 2 and p2hp_max - p2hp > p2hp_max / 2 and p3hp_max - p3hp > p3hp_max / 2 and p4hp_max - p4hp > p4hp_max / 2 and p1hp > 0 and p2hp > 0 and p3hp > 0 and p4hp > 0 then
		r = math.random(3)
		if r == 1 then
	...
	elseif r == 3 then
		--ドレンは1回まで、ムージルの状態でない
		if doren_count < 1 and  p2st < 64 then
			doren()
		else
			pattern_01() --再度行動選択
		end
...
function failure()
	...
	--全員が倒れた場合、失敗
	if p1hp == 0 and p2hp == 0 and p3hp == 0 and p4hp == 0 then
		fail_flag = true
	end
