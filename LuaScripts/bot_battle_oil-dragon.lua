...
local max_frame = 380000 --戦闘終了時の基準フレーム
local max_search = 2000 --戦闘開始を遅らせて試行を始める最大フレーム数
...
function failure()
	...
	if p4st ~= 0 then
		fail_flag = true
		--print("julina is dead")
	end
	
	--主人公、アグロスともに倒れた場合失敗
	if p1st ~= 0 and p3st ~= 0 then
		fail_flag = true
		--print("hero and agros are dead")
	end
