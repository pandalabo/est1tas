...
local max_frame = 17000 --戦闘終了時（敵HPが0になった瞬間）の基準フレーム
local max_search = 200 --戦闘開始を遅らせて試行を始める最大フレーム数

local try_count = 100 --1つの戦闘開始フレームに対し、BOT戦闘を試行する回数
...
function pattern_01()
	...
	if r == 1 then
		...
		if p1hp > 0 and power_drug00_count < 5 then
			power_drug00()	
		else
			pattern_01() --再度行動選択
		end
	elseif r == 2 then
		...
		if p3hp > 0 and power_drug01_count < 5 then
			power_drug01()	
		else
			pattern_01() --再度行動選択
		end
...
function failure()
	...
	if p1hp == 0 and p2hp == 0 and p3hp == 0 and p4hp == 0 then	--ルフィア以外
	--if p4hp == 0 then --ジュリナのみ
		fail_flag = true
...
function success()
	...
	p3hp = memory.readword(0x7E1593)
	p4hp = memory.readword(0x7E1595)
	if p1hp == 0 or p3hp == 0 or p4hp == 0 then --撃破率が低すぎるのでとりあえずルフィア以外に
		return false
	end
